<script lang="ts">
  import { fetchGuiState, setModuleSetting, setModuleStatus } from '../api';
  import type { Category, ModuleEntry, SettingEntry } from '../types';
  ...
  async function loadState(moduleName = selectedModule) {
    try {
      const data = await fetchGuiState(moduleName);
  ...
  async function toggleModule(module: ModuleEntry, state = !module.enabled) {
    ...
    const data = await setModuleStatus(module.name, state);
    if (data.success) {
  ...
  async function updateSetting(setting: SettingEntry, value: string | boolean | number | string[]) {
    ...
    const data = await setModuleSetting(selectedModule, setting.name, raw);
    if (data.success) {
